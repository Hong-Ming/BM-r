function [Uopt,lb,fopt,fval] = bm_full_linf(Weight, Bias, Cost, Image, Radius, Params)
% [UOPT, LB, FOPT, FVAL] = BM_FULL_LINF(Weight, Bias, Cost, Image, Radius, Params)
% 
% BM_FULL_LINF  Use Knitro to solve (BM-Full-linf), which is a rank-r restricted SDP 
%               relaxation of NN verification problem.
% 
% >  [UOPT] = BM_FULL_LINF(Weight, Bias, Cost, Image, Radius, Params) yields an
%       local optimal solution to the rank-r Burer Monteiro reformulation of SDP
% 
% >  [UOPT, LB] = BM_FULL_LINF(...) also returns the lower bound on pgd.
%           LB = <Cost,u{ell}> + Offset - epsilon_gap - epsilon_feas*||Uopt||^2
% 
% >  [UOPT, LB, FOPT] = BM_FULL_LINF(...) also returns the optimal objective value 
%       in the struct array FOPT.
%       FOPT has the following fields:
%       (1) FOPT.pgd         - optimal objective value of PGD.
%       (2) FOPT.bm          - optimal objective value of (BM-Full-linf).
%       (3) FOPT.esilon_gap  - duality gap of (BM-Full-linf).
%       (4) FOPT.esilon_feas - minimum eigenvalue of S(y,z).
% 
% >  [UOPT, LB, FOPT, FVAL] = BM_FULL_LINF(...) also store the history of objective
...
function [g,h,dg,dh] = mycon(variables)
    ...
    err  = [xV(:,1)-Center, xV(:,2:end)];
    pre  = [Wmat*xV(:,1)+bvec, Wmat*xV(:,2:end)]; 
    post = xV(kep,:); % Postactivation
...
function [H] = myhess(~, lambda)
    ...
    H = diag(sparse(1,[1:nx,kep],2*[lambda_g;lambda_h],1,nx,nx+nk));
    H(:,kep) = H(:,kep) - WmatT*L;
    H = kron(speye(r), H);
end
...
function [S] = S_mat(lambda)
    ...
    S = diag(sparse(1,[1:nx,kep],[lambda_g;lambda_h],1,nx,nx+nk));
    S(:,kep) = S(:,kep) - WmatT*L;
    S = S + S';
end

function [xopt,fopt,fval] = pgd_linf(Weight, Bias, Cost, Image, Radius, Params)
% [XOPT, FOPT, FVAL] = PGD_LINF(Weight, Bias, Cost, Image, Radius, Params)
% 
%  PGD_LINF  Find a strong first-order attack for neural networks.
% 
...
% 
% >  [XOPT] = PGD_LINF(Weight, Bias, Cost, Image, Radius, Params) yields an local 
...
% 
% >  [XOPT, FOPT] = PGD_LINF(...) also returns the optimal objective value FOPT.
% 
% >  [XOPT, FOPT, FVAL] = PGD_LINF(...) also store the history of objective
...
%--------------------------------------------------------------------------
ni  = size(Weight{1},2);
ub = min(Params.UB(1:ni),Image+Radius);
lb = max(Params.LB(1:ni),Image-Radius);
...
function debug_proj()
    dif1 = norm(variable-Image,inf)-Radius;
